...
private:
	...
	static GP_BOOL IsConjunction(CMemoryPool *mp, CExpression *predicate_expr);

	// is the condition a boolean predicate
	static GP_BOOL IsPredBooleanScIdent(CExpression *predicate_expr);

	// is the condition a point predicate
	static GP_BOOL IsPointPredicate(CExpression *predicate_expr);

	// is the condition an IDF point predicate
	static GP_BOOL IsPredPointIDF(CExpression *predicate_expr);

	// is the condition an INDF point predicate
	static GP_BOOL IsPredPointINDF(CExpression *predicate_expr);

	// is the condition 'is null' on a scalar ident
	static GP_BOOL IsPredScalarIdentIsNull(CExpression *predicate_expr);

	// is the condition 'is not null' on a scalar ident
	static GP_BOOL IsPredScalarIdentIsNotNull(CExpression *predicate_expr);
	...
	static GP_BOOL IsJoinPredSupportedForStatsEstimation(
		CExpression *expr,
		CColRefSetArray *
	...
	static GP_BOOL AssignExprsToOuterAndInner(
		CColRefSetArray *
			output_col_refsets,	 // array of output columns of join's relational inputs
...
public:
	...
	static GP_BOOL IsConjOrDisjPred(CStatsPred *pred_stats);

	// is unsupported predicate on unknown column
	static GP_BOOL IsUnsupportedPredOnDefinedCol(CStatsPred *pred_stats);
