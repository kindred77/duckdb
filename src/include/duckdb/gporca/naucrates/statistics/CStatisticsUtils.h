...
private:
	...
	static CBucket *CreateValidBucket(CMemoryPool *mp,
									  ...
									  GP_BOOL is_lower_closed,
									  GP_BOOL is_upper_closed);

	// given lower and upper bound information and their closedness, test if they can form a valid bucket
	static GP_BOOL IsValidBucket(CPoint *bucket_lower_bound,
							  CPoint *bucket_upper_bound, GP_BOOL is_lower_closed,
							  GP_BOOL is_upper_closed);
	...
	static void SplitHistDriver(CMemoryPool *mp,
								...
								const CBucketArray *mcv_buckets,
								CBucketArray *merged_buckets, GP_ULONG *mcv_index,
								GP_ULONG mcv);
	...
	static GP_BOOL CappedGrpColExists(const CStatistics *stats,
								   const ULongPtrArray *grouping_columns);
...
public:
	...
	static void UpdateDisjStatistics(
		...
		UlongToHistogramMap *disjunctive_result_histograms, GP_ULONG last_colid);
	...
	static void AddHistogram(CMemoryPool *mp, GP_ULONG colid,
							 const CHistogram *histogram,
							 UlongToHistogramMap *col_histogram_mapping,
							 GP_BOOL replace_old = false);
	...
	static GP_ULONG GetColId(const CStatsPredPtrArry *stats_preds_array);
	...
	static IStatistics *DeriveStatsForDynamicScan(
		CMemoryPool *mp, CExpressionHandle &expr_handle, GP_ULONG part_idx_id,
	...
	static GP_BOOL IncreasesRisk(CLogical *logical_op);
	...
	static GP_BOOL IsStatsCmpTypeNdvEq(CStatsPred::EStatsCmpType stats_cmp_type);
