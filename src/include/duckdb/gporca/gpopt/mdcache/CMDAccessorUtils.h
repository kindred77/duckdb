...
public:
	...
	static GP_BOOL FCastExists(CMDAccessor *md_accessor, IMDId *mdid_src,
							IMDId *mdid_dest);

	// does a scalar comparison object between given types exist
	static GP_BOOL FCmpExists(CMDAccessor *md_accessor, IMDId *left_mdid,
	...
	static GP_BOOL FCmpOrCastedCmpExists(IMDId *left_mdid, IMDId *right_mdid,
	...
	static GP_BOOL FCommutativeScalarOp(CMDAccessor *md_accessor, IMDId *mdid_op);

	// does scalar operator return NULL on NULL input?
	static GP_BOOL FScalarOpReturnsNullOnNullInput(CMDAccessor *md_accessor,
												IMDId *mdid_op);

	// return True if passed mdid is for GP_BOOL type
	static GP_BOOL FBoolType(CMDAccessor *md_accessor, IMDId *mdid_type);
};
}  // namespace gpopt
