...
#define JOIN_ORDER_DP_THRESHOLD GP_ULONG(10)
#define BROADCAST_THRESHOLD GP_ULONG(10000000)
#define PUSH_GROUP_BY_BELOW_SETOP_THRESHOLD GP_ULONG(10)
#define XFORM_BIND_THRESHOLD GP_ULONG(0)
...
{
private:
	GP_ULONG m_ulMinNumOfPartsToRequireSortOnInsert;

	GP_ULONG m_ulJoinArityForAssociativityCommutativity;

	GP_ULONG m_ulArrayExpansionThreshold;

	GP_ULONG m_ulJoinOrderDPLimit;

	GP_ULONG m_ulBroadcastThreshold;

	GP_BOOL m_fEnforceConstraintsOnDML;

	GP_ULONG m_ulPushGroupByBelowSetopThreshold;

	GP_ULONG m_ulXform_bind_threshold;
...
public:
	// ctor
	CHint(GP_ULONG min_num_of_parts_to_require_sort_on_insert,
		  GP_ULONG join_arity_for_associativity_commutativity,
		  GP_ULONG array_expansion_threshold, GP_ULONG ulJoinOrderDPLimit,
		  GP_ULONG broadcast_threshold, GP_BOOL enforce_constraint_on_dml,
		  GP_ULONG push_group_by_below_setop_threshold, GP_ULONG xform_bind_threshold)
