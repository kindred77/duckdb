...
//---------------------------------------------------------------------------
//	@function:
//		CMDTypeGenericCKDB::CMDTypeGenericCKDB
//
//	@doc:
//		Ctor
//
...
//---------------------------------------------------------------------------
//	@function:
//		CMDTypeGenericCKDB::GetMdidForAggType
//
//	@doc:
...
//---------------------------------------------------------------------------
IMDId *
CMDTypeGenericCKDB::GetMdidForAggType(EAggType agg_type) const
{
	switch (agg_type)
	{
		case EaggMin:
...
//---------------------------------------------------------------------------
CMDName
CMDTypeGenericCKDB::Mdname() const
{
	return *m_mdname;
}
...
//---------------------------------------------------------------------------
//	@function:
//		CMDTypeGenericCKDB::GetMdidForCmpType
//
//	@doc:
...
//---------------------------------------------------------------------------
IMDId *
CMDTypeGenericCKDB::GetMdidForCmpType(ECmpType cmp_type) const
{
	switch (cmp_type)
	{
		case EcmptEq:
...
//---------------------------------------------------------------------------
void
CMDTypeGenericCKDB::Serialize(CXMLSerializer *xml_serializer) const
{
	CGPDBTypeHelper<CMDTypeGenericCKDB>::Serialize(xml_serializer, this);
}

//---------------------------------------------------------------------------
//	@function:
//		CMDTypeGenericCKDB::PGetDatumForDXLConstValdatum
//
//	@doc:
...
//---------------------------------------------------------------------------
IDatum *
CMDTypeGenericCKDB::GetDatumForDXLConstVal(
...
//---------------------------------------------------------------------------
//	@function:
//		CMDTypeGenericCKDB::GetDatumForDXLDatum
//
//	@doc:
...
//---------------------------------------------------------------------------
IDatum *
CMDTypeGenericCKDB::GetDatumForDXLDatum(CMemoryPool *mp,
...
//---------------------------------------------------------------------------
//	@function:
//		CMDTypeGenericCKDB::GetDatumVal
//
//	@doc:
// 		Generate dxl datum
//
//---------------------------------------------------------------------------
CDXLDatum *
CMDTypeGenericCKDB::GetDatumVal(CMemoryPool *mp, IDatum *datum) const
{
	m_mdid->AddRef();
...
//---------------------------------------------------------------------------
//	@function:
//		CMDTypeGenericCKDB::IsAmbiguous
//
//	@doc:
...
//---------------------------------------------------------------------------
BOOL
CMDTypeGenericCKDB::IsAmbiguous() const
{
...
//---------------------------------------------------------------------------
//	@function:
//		CMDTypeGenericCKDB::CreateDXLDatumVal
//
//	@doc:
// 		Create a dxl datum
//
//---------------------------------------------------------------------------
CDXLDatum *
CMDTypeGenericCKDB::CreateDXLDatumVal(CMemoryPool *mp, IMDId *mdid,
									  ...
									  LINT lValue, CDouble dValue)
{
	...
	{
		return CMDTypeGenericCKDB::CreateDXLDatumStatsDoubleMappable(
			mp, mdid, type_modifier, is_null, pba, length, lValue, dValue);
	}

	if (HasByte2IntMapping(md_type))
	{
		return CMDTypeGenericCKDB::CreateDXLDatumStatsIntMappable(
			mp, mdid, type_modifier, is_null, pba, length, lValue, dValue);
	}
...
//---------------------------------------------------------------------------
//	@function:
//		CMDTypeGenericCKDB::CreateDXLDatumStatsDoubleMappable
//
//	@doc:
...
{
	GPOS_ASSERT(CMDTypeGenericCKDB::HasByte2DoubleMapping(mdid));
...
//---------------------------------------------------------------------------
//	@function:
//		CMDTypeGenericCKDB::CreateDXLDatumStatsIntMappable
//
//	@doc:
...
//---------------------------------------------------------------------------
//	@function:
//		CMDTypeGenericCKDB::GetDXLOpScConst
//
//	@doc:
...
CMDTypeGenericCKDB::GetDXLOpScConst(CMemoryPool *mp, IDatum *datum) const
{
...
//---------------------------------------------------------------------------
//	@function:
//		CMDTypeGenericCKDB::GetDXLDatumNull
//
//	@doc:
// 		Generate dxl datum
//
//---------------------------------------------------------------------------
CDXLDatum *
CMDTypeGenericCKDB::GetDXLDatumNull(CMemoryPool *mp) const
{
	m_mdid->AddRef();
	CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
	const IMDType *md_type = md_accessor->RetrieveType(m_mdid);
...
//---------------------------------------------------------------------------
//	@function:
//		CMDTypeGenericCKDB::HasByte2IntMapping
//
//	@doc:
...
//---------------------------------------------------------------------------
BOOL
CMDTypeGenericCKDB::HasByte2IntMapping(const IMDType *mdtype)
{
	IMDId *mdid = mdtype->MDId();
	return mdtype->IsTextRelated() || mdid->Equals(&CMDIdCKDB::m_mdid_uuid) ||
		   mdid->Equals(&CMDIdCKDB::m_mdid_cash) ||
		   IsTimeRelatedTypeMappableToLint(mdid);
}

IDatum *
CMDTypeGenericCKDB::CreateGenericNullDatum(CMemoryPool *mp,
...
//---------------------------------------------------------------------------
//	@function:
//		CMDTypeGenericCKDB::HasByte2DoubleMapping
//
//	@doc:
...
//---------------------------------------------------------------------------
BOOL
CMDTypeGenericCKDB::HasByte2DoubleMapping(const IMDId *mdid)
{
	return mdid->Equals(&CMDIdCKDB::m_mdid_numeric) ||
		   mdid->Equals(&CMDIdCKDB::m_mdid_float4) ||
		   mdid->Equals(&CMDIdCKDB::m_mdid_float8) ||
...
//---------------------------------------------------------------------------
//	@function:
//		CMDTypeGenericCKDB::IsTimeRelatedType
//
//	@doc:
//		is this a time-related type
//---------------------------------------------------------------------------
BOOL
CMDTypeGenericCKDB::IsTimeRelatedType(const IMDId *mdid)
{
	return mdid->Equals(&CMDIdCKDB::m_mdid_date) ||
		   mdid->Equals(&CMDIdCKDB::m_mdid_time) ||
		   mdid->Equals(&CMDIdCKDB::m_mdid_timeTz) ||
		   mdid->Equals(&CMDIdCKDB::m_mdid_timestamp) ||
		   mdid->Equals(&CMDIdCKDB::m_mdid_timestampTz) ||
		   mdid->Equals(&CMDIdCKDB::m_mdid_abs_time) ||
		   mdid->Equals(&CMDIdCKDB::m_mdid_relative_time) ||
		   mdid->Equals(&CMDIdCKDB::m_mdid_interval) ||
		   mdid->Equals(&CMDIdCKDB::m_mdid_time_interval);
}

//---------------------------------------------------------------------------
//	@function:
//		CMDTypeGenericCKDB::IsNetworkRelatedType
//
//	@doc:
//		is this a network-related type
//---------------------------------------------------------------------------
BOOL
CMDTypeGenericCKDB::IsNetworkRelatedType(const IMDId *mdid)
{
	return mdid->Equals(&CMDIdCKDB::m_mdid_inet) ||
		   mdid->Equals(&CMDIdCKDB::m_mdid_cidr) ||
		   mdid->Equals(&CMDIdCKDB::m_mdid_macaddr);
}

IMDId *
CMDTypeGenericCKDB::GetDistrOpfamilyMdid() const
{
...
BOOL
CMDTypeGenericCKDB::IsRedistributable() const
{
...
//---------------------------------------------------------------------------
void
CMDTypeGenericCKDB::DebugPrint(IOstream &os) const
{
	CGPDBTypeHelper<CMDTypeGenericCKDB>::DebugPrint(os, this);
}
